        ...
        if ($user) {
            ...
            if ($user['is_active'] == 1) {
                ...
                if (password_verify($password, $user['password'])) {
					...
					$id=$x['id'];
					if($x['kelas']=='A'){ //Administrator
                        $name = $x['nama'];
                        $this->session->set_userdata('access','x');
                        $this->session->set_userdata('id',$id);
                        $this->session->set_userdata('name',$name);
                        redirect('user');

                    }else if($x['kelas']=='B'){ //Dosen
                        $name = $x['nama'];
                        $this->session->set_userdata('access','xi');
                        $this->session->set_userdata('id',$id);
                        $this->session->set_userdata('name',$name);
                        redirect('user');

                    }else if($x['kelas']=='C'){ //Mahasiswa
                        $name = $x['nama'];
                        $this->session->set_userdata('access','xii');
                        $this->session->set_userdata('id',$id);
                        $this->session->set_userdata('name',$name);
                        redirect('user');
                    }
